...
The custom button bitmaps are stored directly in `deploy\application`, beside the DLLs. NX can resolve them from the custom application root. Each icon includes NX size-variant filenames using the `.sc`, `.lc`, `.2s`, base, and `.2l` suffixes. The `.sc` and `.lc` entries intentionally carry 32-pixel source artwork so high-DPI ribbons do not enlarge low-resolution pixels. The update workflows also register this application directory in the current user's `UGII_BITMAP_PATH` as a fallback; preserve any existing bitmap path entries.
